...
func handlerSort(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method != "POST" {
		...
		for i < 10 {
			...
			if doubleEval == 0 {
				initRank = append(initRank, comper)
				i += 1
			}
		}
		...
		for i := 0; i < len(initRank); i++{
			nameArray = append(nameArray, getName(initRank[i]))
			faceArray = append(faceArray, getFace(initRank[i]))
		}
//		data["Name"] = nameArray
//		data["Face"] = faceArray

		plainName := make([]string, 0)
		for i := 0; i < 10; i++{
			plainName += nameArray[i] + "-"
		}

		plainFace := make([]string, 0)
		for i := 0; i < 10; i++{
			plainFace += faceArray[i] + "-"
		}
